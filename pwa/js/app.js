...
class Sidebar {
  ...
  build(component) {
    ...
    if (!window.navigator.onLine) {
      this.sidebarTitle.innerHTML = this.sidebarTitle.innerHTML + `<p>${this.offlineHintText}</p>`;
    }
...
class SourcesList {
  ...
  getListItems(sources) {
    ...
    let initSources = (isOffline && localStorage.getItem('lastSources')) || configAPI.defaultSources;
      ...
      `<li class="filter__item">
        <label class="filter__label">
          <input class="filter__input" type="checkbox"
            value=${elem.id}
            ${isOffline && 'disabled'}
            ${initSources.includes(elem.id) && 'checked'}
          >
...
class RequestsService {
  ...
  getTopHeadlinesURL(sources) {
    let sourcesList = sources ? sources : this.defaultSources.join(',');
